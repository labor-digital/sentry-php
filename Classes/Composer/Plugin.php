...
namespace Labor\Sentry\Composer;
	...
	public function run() {
		...
		if (empty($sentryConfigFilePath)) {
			if (empty(getenv("BITBUCKET_CLONE_DIR"))) {
				$this->io->write("<warning>Did not activate Sentry.io config file, as neither SENTRY_CONFIG_FILE_LOCATION nor BITBUCKET_CLONE_DIR are set as environment variables!</warning>");
				return;
			}
		...
		if (!file_exists($sentryConfigFilePath)) {
			$this->io->write("<error>Failed to get contents of Sentry.io config file, as it does not exist at: $sentryConfigFilePath</error>");
			return;
		}
		...
		$this->io->write("<info>Moved the Sentry.io config file into the composer source directory at: $configTargetPath</info>");
	}
}
